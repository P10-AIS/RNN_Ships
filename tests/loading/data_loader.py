...
class DataLoader():
    ...
    def _load_run_config_from_cache(self, run_config_path=None):
        ...
        if run_config_path is None:
            run_config_path = os.path.join(
                self._get_cache_dir('x'), 'run_config')
    ...
    def _amend_y_timestamp_selection(self, transformations, timestamps_to_take):
        ...
        :return:
        """
        timestamps_to_take = timestamps_to_take if type(
            timestamps_to_take) == list else [timestamps_to_take]
    ...
    def _amend_hours_out_transformation(self):
        ...
        if self.run_config['model_type'] in ['long_term', 'long_term_fusion']:
            percent_of_timestamps_to_take = self.config.interpolation_time_gap / \
                (self.run_config['time'] * 60)
            number_of_timestamps = int(
                (self.config.length_into_the_future + 1) * percent_of_timestamps_to_take)
            total_number_of_hours = int(self.config.interpolation_time_gap * (self.config.length_into_the_future + 1)
                                        / (60 ** 2))
            y_timestamp_to_take = int(
                self.run_config['hours_out'] / total_number_of_hours * number_of_timestamps) - 1
    ...
    def load_set(self, time_period, sliding_window_method, x_or_y, hard_reload=True, for_analysis=False):
        ...
        if time_period not in ['train', 'valid', 'test']:
            raise ValueError(
                'time_period must be one of "train", "valid", "test"]')
        if sliding_window_method not in ['train', 'test']:
            raise ValueError(
                'sliding_window_method must be one of "train", "test"]')
    ...
    def _save_run_config_to_cache(self):
        ...
        self.run_config.save_to_dir(os.path.join(
            self._get_cache_dir('x'), 'run_config'))
    ...
    def _save_ds_to_cache(self, time_period, sliding_window_method, x_or_y):
        ...
        if type(self.dataset) == list:
            dir = os.path.join(
                dir, f'{time_period}_{sliding_window_method}_{x_or_y}')
            clear_path(dir)
            os.mkdir(dir)
            ...
            path = os.path.join(
                dir, f'{time_period}_{sliding_window_method}_{x_or_y}.npy')
            clear_path(path)
    ...
    def _load_ds_from_cache(self, time_period, sliding_window_method, x_or_y):
        ...
        path1 = os.path.join(
            dir, f'{time_period}_{sliding_window_method}_{x_or_y}')
        path2 = os.path.join(
            dir, f'{time_period}_{sliding_window_method}_{x_or_y}_disk_array')
        if os.path.isdir(path1):
            names = os.listdir(path1)
            # Sort
            names = np.array(names)[np.argsort(
                [int(n.split('.')[0]) for n in names])].tolist()
    ...
    def _identify_transformations(self):
        ...
        desired_num_x_timestamps = int(
            self.run_config['length_of_history'] * 60 / self.run_config['time'] + 1)
        current_num_x_timestamps = self.run_config['original_x_shape'][1]
        if desired_num_x_timestamps < current_num_x_timestamps:
            x_timestamps_to_keep = list(
                range(current_num_x_timestamps)[-desired_num_x_timestamps:])
        ...
        if self.run_config['time_of_day'] == 'hour_day':
            base_datetime_idx = loading._find_current_col_idx(
                'base_datetime', self.run_config['columns'])
        ...
        if self.run_config['sog_cog'] in ['min_median_max', 'median']:
            if self.run_config['sog_cog'] == 'min_median_max':
                columns_to_add = ['sog_min', 'sog_median',
                                  'sog_max', 'cog_min', 'cog_median', 'cog_max']
        ...
        if self.run_config['weather'] == 'ignore':
            weather_columns = self.run_config['columns']['column'][self.run_config['columns']
                                                                   ['column_group'] == 'weather']
            self.run_config['columns'], self.run_config['transformations'] = self._identify_cols_to_delete(
                self.run_config['columns'], self.run_config['transformations'], weather_columns.tolist())

        if self.run_config['destination'] == 'ignore':
            destination_columns = self.run_config['columns']['column'][self.run_config['columns']
                                                                       ['column_group'] == 'destination_cluster']
            self.run_config['columns'], self.run_config['transformations'] = self._identify_cols_to_delete(
                self.run_config['columns'], self.run_config['transformations'], destination_columns.tolist())
        elif self.run_config['destination'] == 'ohe':
            destination_center_columns = [
                'destination_cluster_lat_center', 'destination_cluster_lon_center']
            self.run_config['columns'], self.run_config['transformations'] = self._identify_cols_to_delete(
        ...
        if self.run_config['model_type'] == 'long_term_fusion':
            ...
            self.run_config['recurrent_idxs'] = [loading._find_current_col_idx(
                c, self.run_config['columns']) for c in recurrent_cols]
            if len(self.run_config['recurrent_idxs']) != self.run_config['columns']['being_used'].sum():
                ...
                if self.run_config['fusion_layer_structure'] == 'convolutions':
                    ...
                    weather_idxs = [loading._find_current_col_idx(
                        c, self.run_config['columns']) for c in weather_columns]
        ...
        if self.run_config['sog_cog'] == 'raw' and self.run_config['model_type'] == 'iterative':
            ...
            y_cols = ['lat', 'lon']
        self.run_config['y_idxs'] = [loading._find_current_col_idx(
            c, self.run_config['columns']) for c in y_cols]
    ...
    def _identify_cols_to_add(self, columns, transformations, cols, dtype, extra_info=None):
        ...
        t = {'dataset': ['x', 'y'], 'function': 'add_columns',
    ...
    def _identify_cols_to_delete(self, all_columns, transformations, cols_to_delete):
        ...
        :return:
        """
        columns_to_delete = [loading._find_current_col_idx(
            c, all_columns) for c in cols_to_delete]
    ...
    def _calculate_normalization_factors(self):
        ...
        :return:
        """
        normalization_step = np.where(
            [t['function'] == 'normalize' for t in self.run_config['transformations']])[0][0]
    ...
    def _hard_reload_data(self, time_period, sliding_window_method, x_or_y):
        ...
        if self.run_config['model_type'] == 'iterative' and sliding_window_method == 'test':
            all_timestamps = list(
                range(self.run_config['original_y_shape'][1]))
            transformations = self._amend_y_timestamp_selection(
                transformations, all_timestamps)
    ...
    def _load_data_for_analysis(self, time_period, sliding_window_method, x_or_y):
        ...
        analysis_columns, analysis_transformations = self._identify_analysis_transformations(
            analysis_columns)
    ...
    def _identify_analysis_transformations(self, analysis_columns):
        ...
        base_datetime_idx = loading._find_current_col_idx(
            'base_datetime', analysis_columns)
        ...
        water_u_idxs = [loading._find_current_col_idx(
            c, analysis_columns) for c in water_u_columns]
        water_u_idxs = np.array(water_u_idxs)[
            np.argsort(water_u_columns)].tolist()

        water_v_idxs = [loading._find_current_col_idx(
            c, analysis_columns) for c in water_v_columns]
        water_v_idxs = np.array(water_v_idxs)[
            np.argsort(water_v_columns)].tolist()
        ...
        analysis_columns, analysis_transformations = self._identify_cols_to_add(
            analysis_columns, analysis_transformations,
            ['closest_weather_us', 'closest_weather_vs',
                'closest_weather_lats', 'closest_weather_lons'],
            'object'
        )
        ...
        vg_columns = analysis_columns[analysis_columns['column_group']
                                      == 'vessel_group']['column'].to_list()
        analysis_columns, analysis_transformations = self._identify_cols_to_add(
            analysis_columns, analysis_transformations,
            ['vessel_group'], 'object',
    ...
    def _create_columns_df(self):
        ...
        :return:
        """
        self.run_config['columns'] = pd.read_csv(os.path.join(
            self.run_config['formatted_dir'], 'features.csv'))
    ...
    def _find_shapes(self):
        ...
        Y = loading.read_ts_data(
            data_dir, self.run_config['time'], 'y', dtype='float32', conserve_memory=self.conserve_memory)
