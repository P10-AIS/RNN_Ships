...
class MedianStopper(keras.callbacks.Callback):
    ...
    def __init__(self, run_id, experiment_id, iterations=30):
        self.cutoffs_path = os.path.join(
            config.box_and_year_dir, '.median_losses.csv')
        self.run_id = run_id
    ...
    def on_train_end(self, logs=None):
        ...
        if self.early_stopping_occured:
            print(
                f'Epoch {self.stopped_epoch + 1}: early stopping occurred based on median loss')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if ((epoch + 1) / self.iterations) % 1 == 0:
            if epoch + 1 in self.cutoffs['steps'].values:
                cutoff = self.cutoffs[self.cutoffs['steps']
                                      == epoch + 1]['average_loss'].iloc[0]
            else:
                cutoff = np.inf
    ...
    def _load_cutoffs(self):
        ...
        self.cutoffs = self.cutoffs[self.cutoffs['experiment_id'] == int(
            self.experiment_id)]
    ...
    def recalculate_cutoffs(self):
        ...
        run_info = pd.DataFrame(
            columns=['steps', 'average_loss', 'experiment_id'])
        for run in runs:
            if run.info.status == 'FINISHED' or run.info.run_id == self.run_id:
                ...
                try:
                    loss_path = os.path.join(
                        overall_dir, 'metrics', 'epoch_val_loss')
                    loss_history = pd.read_csv(
                        loss_path, names=['time', 'loss', 'step'], sep='\s+')
                except urllib.error.URLError:
                    loss_path = os.path.join(
                        overall_dir, 'metrics', 'val_loss')
                    loss_history = pd.read_csv(
                        loss_path, names=['time', 'loss', 'step'], sep='\s+')
                loss_history['steps'] = 1
                loss_history = loss_history[['loss', 'steps']].cumsum()
                loss_history['average_loss'] = loss_history['loss'] / \
                    loss_history['steps']
