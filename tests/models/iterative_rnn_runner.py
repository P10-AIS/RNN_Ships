...
class RNNModelRunner(ModelRunner):
    ...
    def _init_model(self):
        ...
        if num_full_sequence_layers > 0:
            if self.direction == 'forward_only':
                hidden.append(self.rnn_layer(self.rnn_layer_size,
                              return_sequences=True)(hidden[-1]))
                num_full_sequence_layers -= 1
            elif self.direction == 'bidirectional':
                hidden.append(Bidirectional(self.rnn_layer(
                    self.rnn_layer_size, return_sequences=True))(hidden[-1]))
                num_full_sequence_layers -= 1
        ...
        if self.rnn_to_dense_connection == 'all_nodes':
            ...
            if self.direction == 'forward_only':
                hidden.append(self.rnn_layer(self.rnn_layer_size,
                              return_sequences=False)(hidden[-1]))
            elif self.direction == 'bidirectional':
                hidden.append(Bidirectional(self.rnn_layer(
                    self.rnn_layer_size, return_sequences=False))(hidden[-1]))

        # Define Dense section
        for layer in range(self.number_of_dense_layers):
            hidden.append(Dense(self.dense_layer_size,
    ...
    def insert_predictions(self, X, predictions, time):
        ...
        Y_hat_i_full = Normalizer().unnormalize(
        ...
        if 'distance_traveled' in self.columns['column'].values:
            lat_lon_idx = [_find_current_col_idx(
                c, self.columns) for c in ['lat', 'lon']]
            first_lat_lon = Normalizer().unnormalize(
                X[:, 1, lat_lon_idx], self.normalization_factors).copy()
            predicted_lat_lon = Y_hat_i_full[:, lat_lon_idx]
            distance_traveled = haversine_vector(
                first_lat_lon, predicted_lat_lon, Unit.KILOMETERS)
        ...
        if 'day_of_week' in self.columns['column'].values:
            ...
            incremented = pd.DataFrame(
                {'r': np.where(incremented)[0], 'c': np.where(incremented)[1]})
            last_incremented = incremented.groupby(
                'r').max().to_numpy().squeeze()
        ...
        Y_hat_i_full = Normalizer().normalize_data(
            Y_hat_i_full, self.normalization_factors)
    ...
    def predict(self, valid_X_long_term, valid_Y_long_term, args):
        ...
        for i in range(valid_Y_long_term.shape[1]):
            ...
            Y_hat_i_unnormalized = Normalizer().unnormalize(
                Y_hat_i_normalized, self.normalization_factors)
            valid_Y_hats.append(Y_hat_i_unnormalized)

            # Get haversine distance
            lat_lon_idxs = [_find_current_col_idx(
                c, self.columns) for c in ['lat', 'lon']]
            ground_truth_Y_unnormalized = Normalizer().unnormalize(
            ...
            if i != valid_Y_long_term.shape[1] - 1:
                valid_Xs.append(self.insert_predictions(
                    X, Y_hat_i_normalized, args.time))

        # TODO: Make more adjustable
        common_prediction_time = 60
        hour_idxs = [int((h - common_prediction_time) /
                         args.time) - 1 for h in [120, 180, 240]]

        valid_Y_hats = [valid_Y_hats[i] for i in hour_idxs]
        hour_haversine_distances = [haversine_distances[i] for i in hour_idxs]
        mean_hour_haversine_distances = np.array(
            mean_haversine_distances)[hour_idxs]
