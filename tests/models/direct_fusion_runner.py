...
from keras.layers import GRU as GRUKeras, LSTM as LSTMKeras, Bidirectional, Concatenate, Flatten, Conv2D, MaxPooling2D, Dense, \
    Dropout
...
class FusionModelRunner(ModelRunner):
    ...
    def _init_model(self):
        ...
        """
        # Recurrent section
        recurrent_input = Input(shape=(
            self.ts_length, self.input_num_recurrent_features), name='recurrent_input')
        ...
        if self.rnn_to_dense_connection == 'all_nodes':
            recurrent_layers.append(
                Flatten('channels_first')(recurrent_layers[-1]))
        else:
            if self.direction == 'forward_only':
        ...
        if self.fusion_layer_structure == 'convolutions':
            weather_input = Input(
                shape=self.weather_shape[1:], name='weather_input')
            ...
            initial_weather_layers.append(
                Flatten('channels_last')(initial_weather_layers[-1]))

        elif self.fusion_layer_structure == 'dense':
            weather_input = Input(
                shape=(self.input_num_dense_features,), name='weather_input')
            initial_weather_layers = [weather_input]
            for layer in range(self.number_of_fusion_weather_layers):
                ...
                initial_weather_layers.append(
                    Dropout(self.dense_dropout)(initial_weather_layers[-1]))

        if self.fusion_layer_structure is None:
            final_layers = [recurrent_layers[-1]]
        else:
            final_layers = [
                Concatenate(axis=1)([recurrent_layers[-1], initial_weather_layers[-1]])]
        ...
        if self.fusion_layer_structure is None:
            ...
            self.model = Model(
                inputs=[recurrent_input, weather_input], outputs=output)
